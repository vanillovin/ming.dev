...
특정 액션이 디스패치되면 스토어의 상태가 업데이트되고, 상태가 업데이트되면 우리가 구독했던 함수가 호출됨. unsubscribe 하면 호출이 되지 않음. 우리가 앞으로 리액트에서 리덕스를 사용할 때는 subscribe 함수는 사용하지 않음. 그리고 getState도 직접적으로 사용하는 일은 미들웨어말고는 없음. helper hooks를 사용해 리덕스와 리액틀를 연동
